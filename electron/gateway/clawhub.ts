...
export class ClawHubService {
    ...
    constructor() {
        ...
        const packagedNodePath = path.join(process.resourcesPath || '', 'bin', process.platform === 'win32' ? 'node.exe' : 'node');
        if (app.isPackaged && fs.existsSync(packagedNodePath)) {
            this.cliPath = packagedNodePath;
            this.useNodeRunner = true;
        } else if (!app.isPackaged && fs.existsSync(binPath)) {
    ...
    private async runCommand(args: string[]): Promise<string> {
        return new Promise((resolve, reject) => {
            ...
            const child = spawn(spawnCmd, spawnArgs, {
                ...
                env: {
                    ...env,
                    CLAWHUB_CONFIG_PATH: path.join(this.workDir, '.clawhub', 'config.json'),
