...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  // UI-created tasks have no delivery target — results go to the BajaClaw chat page.
  ...
  ipcMain.handle('cron:create', async (_, input: {
    ...
    try {
      const gatewayInput = {
        ...
        wakeMode: 'next-heartbeat',
        sessionTarget: 'isolated',
        // UI-created jobs deliver results via BajaClaw WebSocket chat events,
...
function registerUvHandlers(): void {
  ...
  // Verify bundled uv. BajaClaw ships with OpenClaw already built, so first run
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        console.log(`[baja-claw-validate] validating provider type: ${providerType}`);
 ...
 * Stage files to BajaClaw's isolated OpenClaw media/outbound directory for gateway access
 */
