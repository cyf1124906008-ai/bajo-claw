...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  // UI-created tasks have no delivery target — results go to the Bajo Claw chat page.
  ...
  ipcMain.handle('cron:create', async (_, input: {
    ...
    try {
      const gatewayInput = {
        ...
        wakeMode: 'next-heartbeat',
        sessionTarget: 'isolated',
        // UI-created jobs deliver results via Bajo Claw WebSocket chat events,
...
function registerUvHandlers(): void {
  ...
  // Verify bundled uv. Bajo Claw ships with OpenClaw already built, so first run
 ...
 * Stage files to Bajo Claw's isolated OpenClaw media/outbound directory for gateway access
 */
